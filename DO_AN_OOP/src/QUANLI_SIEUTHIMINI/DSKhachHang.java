...
public class DSKhachHang implements InterfaceC {
	public static int SoLuong = 0;
	private static KHACHHANG[] dskh = new KHACHHANG[SoLuong];
	
	static int countVIP = 0;
	static int countNOR = 0;
	...
	public void xuat() {
		...
		String out1 = String.format("|%-15s|%-15s|%-15s|%-15s|%-40s|%-20s|%-15s|", "ID Khach Hang", "Ten Khach Hang",
				"Mat khau", "So dien thoai", "Dia Chi", "loai khach hang", "Muc giam gia");
	...
	public void themkh(int k) {
		dskh = Arrays.copyOf(dskh, SoLuong + k);
	...
	public void ThemO() {
		if (SoLuong == 0) {
			...
			for(int i = 0; i < SoLuong; ++i) {
				if(dskh[i].getMaKH().contains("VIP")) {
					countVIP++;
				}else {
					countNOR++;
				}
	...
	public void xoatheomakh(String x) {
		int kt = -1;
		for (int i = 0; i < SoLuong; ++i) {
			if (dskh[i].getMaKH().equals(x)) {
				kt = i;
				break;
			}
		}
		if (kt == -1) {
			System.out.println("Ma kh nay ko ton tai!!");
		} else {
			xoa(kt);
	...
	public void SuaO() {
		...
		while (check) {
			try {
				System.out.println("__Nhap ma khach hang can sua__");
				ma = sc.next();
				for (int i = 0; i < SoLuong; i++) {
				...
				if (check == true) {
					System.out.println("Ma khach hang khong ton tai, nhap lai!!");
				}
			} catch (Exception e) {
				System.out.println("Ma khach hang khong ton tai, nhap lai!!");
			}
	...
	public void docFile() {
		try {
			FileReader fr = new FileReader("C:\\Users\\NGOC THUC\\eclipse-workspace\\DO_AN_OOP\\data\\KHACHHANG.txt");
			...
			while (true) {
				...
				String tquan = s[7];
				String loaiKH = s[8];
				double ptgiamgia = Double.parseDouble(s[9].trim());

//				System.out.println(hoTen);

				dskh = Arrays.copyOf(dskh, SoLuong + 1);
				dskh[SoLuong] = new KHACHHANG(maKH, hoTen, ngaysinh, sdt, sonha, tduong, sophuong, tquan, loaiKH,
						ptgiamgia);
//				System.out.println("Ma khach hang" + String.valueOf(dskh[SoLuong].theThanhVien.getMaKH().toString()));
				SoLuong++;
			}
			br.close();
			fr.close();
	...
	public void ghiFile() {
		try {
			FileWriter fw = new FileWriter(
					"C:\\Users\\NGOC THUC\\eclipse-workspace\\DO_AN_OOP\\WriteFile\\KHACHHANG.txt");
	...
	public static boolean checkMaKHexist(String x) {
		if (SoLuong == 0) {
			System.out.println("Chua co khach hang");
			return false;
		}
		for (int i = 0; i < dskh.length; i++) {
			if (dskh[i].getMaKH().equals(x.trim()))
				return true;
		}
		return false;
	}
